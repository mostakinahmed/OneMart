...
long long getInvoiceNumber()
{
    FILE *fp = fopen("sales/invoice_number.txt", "r");
    long long number = 0;
    fscanf(fp, "%llu", &number);
    number++;

    // Move to the beginning of the file and overwrite
    freopen("sales/invoice_number.txt", "w", fp);
    fprintf(fp, "%llu", number);
    fclose(fp);
...
void newSales()
{
    ...
    printCentered("New Sale", 11);
    printCentered("=====================", 15);
    printf("\n\n");

    int pID2, productIndex = 0;
    ...
    // Picking product index number from file
    int index;
    FILE *fp;
    ...
    if (found)
    {
        ...
        printCentered("  ----------------------", 10);
        printCentered("  Product-ID:        Product-Name:   Product-Price:       Available:   Category:", 15);
        printCentered("  --------------------------------------------------------------------------------", 9);
        ...
        if (pUnit > allProduct[productIndex].pUnit)
        {
        ...
        {
            printCentered("Invalid quantity!", 10);
            printCentered("Press any key to try again...", 10);
            _getch();
            newSales();
        }
        ...
        printf("\n");
        int totalPrice = pUnit * allProduct[productIndex].pPrice;
        ...
        printf("                                        %d         %s              %d      X       %d               %d.00\n",
               allProduct[productIndex].pID, allProduct[productIndex].pName, pUnit, allProduct[productIndex].pPrice, totalPrice);
        printCentered("  ------------------------------------------------------------------------------", 15);
        printCentered("                                                     ---------------------------", 4);
        ...
        for (int i = 0; i < space; i++)
            printf(" ");
        printf("Input Sale Date (day mon year): ", 15);
        ...
        int payOption;
        int oflineCustomerID = 11111;
        char oflineTransactionNum[15] = "xxxxxxxxxxxx";
        char oflineCustomerName[10] = "ABCDE";

        char offline[10] = "offline";
        printCentered("Payment Method....", 10);
        printCentered("   1. Cash       ", 15);
        printCentered("  2. OnePay-Card", 15);
        ...
        case 1:
            ...
            printf("                                                              Product Name   :     %s\n", allProduct[productIndex].pName);
            printf("                                                              Category       :     %s\n", allProduct[productIndex].pCat);
            printf("                                                              Quantity       :     %d\n", pUnit);
            printCentered("--------------------------------", 10);
        ...
        for (int i = 0; i < index; i++)
        {
            if (strcmp(mobileBanking[i].accountNumber, accountNumber2) == 0 && (strcmp(mobileBanking[i].bankName, payOption) == 0))
            {
                ...
                if (mobileBanking[i].pin == pin2)
                {
                    ...
                    if (mobileBanking[i].balance >= rechargeAmount2)
                    {
                        ...
                        mobileBanking[i].balance -= rechargeAmount2;
                        fp = fopen("mobile_banking/all_account.txt", "w");
                        fclose(fp);
                        fp = fopen("mobile_banking/all_account.txt", "a");
                        for (int j = 0; j < index; j++)
                        {
                            fprintf(fp, "%s %d %.2f %s\n",
                                    mobileBanking[j].accountNumber, mobileBanking[j].pin,
                                    mobileBanking[j].balance, mobileBanking[j].bankName);
                        }
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < space; i++)
                            printf(" ");
                        printf("Insufficient balance in your %s account.\n", payOption);
                        printCentered("Press any key to try again......", 4);
                        _getch();
                        rechargeByMobileAndCard(payOption);
                    }
...
void dailyIncome()
{
    ...
    for (i = 0; i < index; i++)
    {
        if (currentDate.day == allSalesProduct[i].saleDate.day && currentDate.mon == allSalesProduct[i].saleDate.mon && currentDate.year == allSalesProduct[i].saleDate.year)
        {
            found = 1;
            break;
        }
...
void buyProduct()
{
    ...
    if (found)
    {
        ...
        else
        {
            ...
            printCentered("Choose Option: ", 10);
            ...
            if (optionForPromo == 1)
            {
                ...
                // Check promo code
                if (strcmp(promoCode, "PROMO10") == 0)
                {
                    total = total - (total * 0.15);

                    printf("       Prev total : %.2f               Total           : %.2f\n\n", total + (total * 0.15), total);
            ...
            if (optionForPay == 1)
            {
                printf("You have selected OnePay Card:\n");
                printCentered("  Your Card Info:", 10);
                printCentered("  ----------------------", 10);
...
void OnlineHome()
{
    ...
    case 2:
        ...
        default:
            printCentered("Invalid Choice! 2", 4);
            _getch();
            OnlineHome();
            break;
        }
